...
for IDX in `seq 1 $NUM_CALLS`; do
    ...
    READV_CLOSE=${IDX} LD_PRELOAD=./ld_sockerr.so libtool --mode=execute valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=77 ../utils/smb2-ls "${TESTURL}/" >/dev/null 2>valgrind.out
