...
#else
#define DEFAULT_OUTPUT_BUFFER_LENGTH 2048
#endif
...
#else /* !defined(DISABLE_LOGOFF_CMD) */
        pdu = smb2_cmd_tree_disconnect_async(smb2, disconnect_cb_2, dc_data);
#endif /* !defined(DISABLE_LOGOFF_CMD) */
        if (pdu == NULL) {
                free(dc_data);
                return -ENOMEM;
