...
SOCKET GetSocketForIndex(int iIndex)
{
  ...
  {
    printf("GetSocketForIndex() invalid index: %i", iIndex);
  ...
  {
	  printf("GetSocketForIndex() invalid socket for index: %i", iIndex);
  }
...
int AddSocket(SOCKET iSocket)
{
  ...
  printf(__FUNCTION__ " - Unable to add socket to internal list, no space left");
  return -1;
}
...
extern "C"
{
...
#ifdef _XBOX
  struct mphostent* __stdcall sckemu_gethostbyname(const char* name) // TODO: Not currently used or needed
  {
	  printf("Socket Emu - gethostbyname() not implemented.");
    ...
    sckemu_gethostname(hbn_hostname, 128);
  ...
  {
    return (hostent*)sckemu_gethostbyname(name);
#endif
  }
#else
...
//    sprintf(szBuf, "connect: family:%i port:%i ip:%i.%i.%i.%i\n",
 //           pTmp->sin_family,
  ...
  int __stdcall sckemu_send(int s, const char *buf, int len, int flags)
  {
    SOCKET socket = GetSocketForIndex(s);
...
//      CLog::Log(LOGDEBUG, "sckemu_bind: family:%i port:%i ip:%i.%i.%i.%i\n",
//            address->sin_family,
 //           ntohs(address->sin_port),
  ...
  int __stdcall sckemu_ioctlsocket(int s, long cmd, DWORD FAR * argp)
  {
    ...
    {
      printf("sckemu_ioctlsocket invalid index:%i", s);
      return SOCKET_ERROR;
    }
  ...
  int __stdcall sckemu_recv(int s, char FAR * buf, int len, int flags)
  {
    if (s < 3 || s >= MAX_SOCKETS)
    {
      printf("sckemu_recv invalid index:%i", s);
      return SOCKET_ERROR;
    }
    SSocketData& socket = m_sockets[s];
    int len2, len3;
    ...
    if(flags & MSG_PEEK)
    {
      printf(__FUNCTION__" - called with MSG_PEEK set, attempting workaround");
...
//      len2 = std::min(len, socket.end - socket.start);
    ...
    if(flags)
      printf(__FUNCTION__" - called with flags %d that will be ignored", flags);
    flags = 0;
...
//      len2 = std::min(len, socket.end - socket.start);
  ...
  int __stdcall sckemu_WSAFDIsSet(int s, fd_set* set)
  {
...
//    sprintf(_inetaddress, "%d.%d.%d.%d", in.S_un.S_un_b.s_b1, in.S_un.S_un_b.s_b2, in.S_un.S_un_b.s_b3, in.S_un.S_un_b.s_b4);
  ...
  {
    printf("Untested function sckemu_gethostbyaddr called!");
    XNADDR xna;
    DWORD dwState;
    ...
    {
      //get title hostent
      sckemu_gethostname(hba_hostname, 128);
    ...
    else
    {
      ...
      strcpy(hba_hostname, sckemu_ntoa(client));
